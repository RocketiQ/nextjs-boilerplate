...
export default function CareersIndex() {
  ...
  .careers-card::before {
    content: '';
    position: absolute;
    inset: -10px; /* let the glow breathe outside the card */
    border-radius: 18px;
    background:
      radial-gradient(
        90% 120% at 50% 55%,              /* <-- moved from 50% 10% to center(ish) */
  ...
  /* Subtle border halo so the whole edge brightens uniformly */
  .careers-card::after {
    content: '';
    position: absolute;
    inset: 0;
  ...
  /* CTA glow (unchanged, keeps the nice button pop) */
